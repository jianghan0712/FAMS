...
public class GeneratePythonBOFiles {
	...
	private StringBuilder analysis(String line, int i) {
		...
		if (str[2].equalsIgnoreCase("uuid") || str[2].equalsIgnoreCase("boid")
				|| str[2].equalsIgnoreCase("destination")) {
			fin.append("    required ");
		} else {
			fin.append("    optional ");
		}
...
//			    	generateProtoFile(f);
	...
	private void genOTWContent(String boName) throws ClassNotFoundException {
		...
		println(new StringBuilder("from ").append("src.").append(bopackageName).append(".pro import ").append(_pb2Name)
				.toString());
		println(new StringBuilder("from ").append("src.").append(bopackageName).append(".model.").append(boName)
				.append(" import ").append(boName).toString());
		println("");
		...
		println(new StringBuilder(TAB).append(TAB).append("self._bo_pro = ").append(_pb2Name).append(".").append(boName)
				.append("()").toString());
		...
		println(new StringBuilder(TAB).append(TAB).append(TAB).append("self._bo_pro.ParseFromString(byteMsg)")
				.toString());
		println(new StringBuilder(TAB).append(TAB).append(TAB).append("self.__setDataFromPB()").toString());
		println(new StringBuilder(TAB).append(TAB).append("else:").toString());
		println(new StringBuilder(TAB).append(TAB).append(TAB).append("self.__setDataFromBO()").toString());
		println("");
	...
	private void genSetMethod(Field field) {
		...
		println(new StringBuilder(TAB).append("def set").append(part1).append(part2).append("(self, ").append(name)
				.append("):").toString());
		println(new StringBuilder(TAB).append(TAB).append("self._bo.").append(name).append(" = ").append(name)
				.toString());
		println(new StringBuilder(TAB).append(TAB).append("self._bo_pro.").append(name).append(" = ").append(name)
				.toString());
		println("");
	}
	...
	private void genDataFromPB(Field[] fields) {
		...
		for (Field each : fields) {
			println(new StringBuilder(TAB).append(TAB).append("self._bo.").append(each.getName()).append(" = ")
					.append("self._bo_pro.").append(each.getName()).toString());
		}
	}
	...
	private void genDataFromBO(Field[] fields) {
		// TODO Auto-generated method stub
		for (Field each : fields) {
			println(new StringBuilder(TAB).append(TAB).append("self._bo_pro.").append(each.getName()).append(" = ")
					.append("self._bo.").append(each.getName()).toString());
		}
	}
	...
	private void genToString(String name, Field[] fields) {
		...
		for (Field field : fields) {
			...
			if (!String.class.equals(field.getType())) {
				all.append("\"").append(field.getName()).append(" = \" + str(self.").append(methodName).append("()) +")
						.append("\",\" +");
			} else {
				all.append("\"").append(field.getName()).append(" = \" + self.").append(methodName).append("() +")
						.append("\",\" +");
			}
		}
		all.append("\"]\"");
...
//				boid = 4L, destination = "fstp.core.manager.qnsrespond"
		...
		for (Field e : fileds) {
			if (e.getName().equalsIgnoreCase("uuid")) {
				...
				println(new StringBuilder().append(TAB).append(TAB).append("self.destination = \"").append(des)
						.append("\"").toString());
			} else if (e.getType().equals(java.lang.String.class)) {
				println(new StringBuilder().append(TAB).append(TAB).append("self.").append(e.getName()).append(" = ''")
						.toString());
			} else if (e.getType().equals(double.class)) {
				println(new StringBuilder().append(TAB).append(TAB).append("self.").append(e.getName()).append(" = 0.0")
						.toString());
			} else if (e.getType().equals(boolean.class)) {
				println(new StringBuilder().append(TAB).append(TAB).append("self.").append(e.getName())
						.append(" = False").toString());
			} else if (e.getType().equals(float.class)) {
				println(new StringBuilder().append(TAB).append(TAB).append("self.").append(e.getName()).append(" = 0.0")
						.toString());
			} else if (e.getType().equals(int.class)) {
				println(new StringBuilder().append(TAB).append(TAB).append("self.").append(e.getName()).append(" = 0")
						.toString());
			} else if (e.getType().equals(long.class)) {
				println(new StringBuilder().append(TAB).append(TAB).append("self.").append(e.getName()).append(" = 0L")
						.toString());
			}
		}
	}
...
//		String strCmd = "./resource/protoc.exe -I=./" + protofileDirectory + " --python_out=./" + builderfileDirectory + " ./" + protofileDirectory + fileName;  
		String strCmd = "D:\\software\\protoc-3.11.4-win64\\bin\\protoc.exe -I=./" + protofileDirectory
				+ " --python_out=./" + builderfileDirectory + " ./" + protofileDirectory + fileName;
		...
		if (flist == null || flist.length == 0) {
			...
			for (File f : flist) {
				if (f.isDirectory()) {
					...
					System.out.println(targetPath + "----" + className + "----" + protofileDirectory + "----"
							+ otwfileDirectory + "----" + builderfileDirectory + "-----" + bopackageName);
					File tardir = new File(protofileDirectory);
					if (!tardir.exists()) {
						tardir.mkdirs();
					}
