    ...
    def __getStockList(self): 
        ...
        if self.config['stockcode'] != '' and self.config['industry'] =='': 
            ...
            for i in self.config['stockcode'].split(','):  
                sql = sql + "'" + i + "',"
            print(sql)           
            stockCodeList = self.mysql.getData("select ts_id as stockcode,industry from fams_security_basicinfo where ts_id in (" + sql.strip(',') + ")") 
        ...
        if self.config['stockcode'] == '' and self.config['industry'] !='':
            ...
            for i in self.config['industry'].split(','):  
                sql = sql + "'" + i + "',"
            print(sql)  
            stockCodeList = self.mysql.getData("select ts_id as stockcode,industry from fams_security_basicinfo where industry in (" + sql.strip(',') + ")")   
